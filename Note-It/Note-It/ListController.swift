...
class ListController: UITableViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        try? Notes.sharedInstance.add(note: Note(title: "ss", text: "ggg"))
        try? Notes.sharedInstance.add(note: Note(title: "BB", text: "ggg"))
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let label = cell.textLabel, let note = try? Notes.sharedInstance.getNote(atIndex: indexPath.row) {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showNote" {
            print ("segue with \(segue.identifier) Identified triggered" )
